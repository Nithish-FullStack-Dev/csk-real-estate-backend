...
export const getMyPurchase = asyncHandler(async (req, res) => {
  ...
  const purchases = await Customer.find(filter)
    ...
    .populate("openPlot", "_id projectName openPlotNo thumbnailUrl location")
    .populate("innerPlot", "_id plotNumber thumbnailUrl location")
    .populate("openLand", "_id projectName surveyNumber thumbnailUrl location")
    .sort({ createdAt: -1 })
    .skip(skip)
    .limit(limit)
    .lean();
